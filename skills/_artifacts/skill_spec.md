...
Shared Cloudflare ingress for ephemeral cloud development environments. One Worker plus Durable Objects; an npm sidecar reverse-tunnels local HTTP servers (or optionally registers a public HTTPS target). Skills target library version 0.3.2.
...
| --- | --- | --- | --- | --- |
| connect | core | client-connection | CLI, DevRouterClient, reverse tunnel, control server, replica-mode OAuth prompt | 7 |
...
None for 0.3.2 reverse-tunnel Host-from-localUrl, live `/ready`, OAuth correlation, control server, environment identity, Amp example, or scoped join credentials.
